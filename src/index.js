...
function updateAria(event) {
  ...
  } else if (event.target.id === 'manualDateSelector') {
...
function updateCalendar(date) {
  calendarManager.setAttribute('min', date.replaceAll('/', '-'));
  calendarManager.setAttribute('value', date.replaceAll('/', '-'));
...
function managerBookRoom(roomInfo, button) {
  fetch(`http://localhost:3001/api/v1/bookings`, {
    ...
    .catch(err => managerServerMessage());
}
...
function checkForLoginError(response, error) {
  if (!response.ok) {
...
function searchGuests() {
  ...
  if (manager.selectedUser === '') {
    displaySelectedUserMsg();
  } else {
    updateSelectedUser(); 
  }
}

function displaySelectedUserMsg() {
  show(selectedUserErrorMsg);
  selectedUserName.innerText = "Find Guest";
...
function updateSelectedUser() {
  ...
  selectedUserName.innerText = manager.selectedUser.name; 
...
function filterRoomsByDate(event) {
  const filterButtons = document.querySelectorAll('.room-type');
